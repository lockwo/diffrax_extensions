...
See also the documentation for [`diffrax_extensions.AbstractSRK`][] for 
additional details on the mathematical meaning of each of these arguments.
...
class AbstractSRK(AbstractSolver[_SolverState]):
    ...
    For example, the [`diffrax_extensions.ShARK`][] solver requires space-time
    Lévy area, so it will have `minimal_levy_area = AbstractSpaceTimeLevyArea` and
    the Brownian motion must be initialised with `levy_area=SpaceTimeLevyArea`.
    ...
    The coefficients are provided in the
    [`diffrax_extensions.StochasticButcherTableau`][]. In particular the coefficients
    $b^W$, and $a^W$ are provided in `tableau.cfs_bm`, as well as $b^H$, $a^H$, $b^K$,
    and $a^K$ if needed.
    """
